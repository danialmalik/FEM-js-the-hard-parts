...
// CHALLENGE 1
/**
 * A) Create a for loop that iterates through an array and
 * returns the sum of the elements of the array.
 *
 * B) Create a functional iterator for an array that
 ...
 * each value of the array when .next is called.
 */
...
// CHALLENGE 4
/**
 * Create an iterator with a next method that returns each value of
 * a set when .next is called
 */
 ...
 * elements (where the first element is the index and the second is the
 * value at that index) when .next is called.
 */
function indexIterator(arr) {
    let curr = 0;
    return {
        next() {
            return [curr, arr[curr++]];
        }
    }
 ...
 * on the call of its .next method (hint: use regex!)
 *
 * Then attach it as a method to the prototype of a constructor Words.
...
Words.prototype[Symbol.iterator] = function () {
    const words = this.str.match(/(\w+)/g);
    let curr = -1;
    return {
        next() {
            return {
                value: words[++curr],
                done: curr === words.length
            }
        }
    }
...
// CHALLENGE 7
/**
 * Build a function that walks through an array and returns the
 * element concatenated with the string "was found after index x",
...
function valueAndPrevIndex(array) {
    ...
    return {
        sentence: function () {
            ...
            return `${array[index++]} is found after index ${index}`;
        }
    }
}
 ...
 * verb in which the non async function takes in a noun, concatenates
 * it with a hard coded verb and returns it to the async function to be
 * console.logged after a duration of 3 seconds.
...
async function f(noun) {
    const sentence = await new Promise((resolve) => {
        setTimeout(() => resolve(waitForVerb(noun)), 3000);
    });
